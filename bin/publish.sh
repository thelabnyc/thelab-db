...
# Set version in thelabdb/__init__.py
PYINIT_FILE="thelabdb/__init__.py"
PYINIT_BK_FILE="${PYINIT_FILE}.bk"
awk "{gsub(/^__version__ = (.+)$/,\"__version__ = \\\"$VERSION\\\"\")}1" "$PYINIT_FILE" > "$PYINIT_BK_FILE"
mv "$PYINIT_BK_FILE" "$PYINIT_FILE"
git add "$PYINIT_FILE"
